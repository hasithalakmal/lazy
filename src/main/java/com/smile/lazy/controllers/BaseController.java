...
import com.smile.lazy.exception.CustomErrorBean;
...
public class BaseController {
    ...
    public CustomErrorBean handleException(Exception e, HttpServletResponse response) {
        LOGGER.error(e.getMessage(), e);
        return getError(HttpStatus.INTERNAL_SERVER_ERROR.value(), ErrorCodes.UNKNOWN_SERVER_ERROR, UNKNOWN_ERROR_MSG, e.getMessage(), response);
    }

    private CustomErrorBean getError(int httpStatusCode, String errorCode, String description, String additionalInfo, HttpServletResponse response) {
