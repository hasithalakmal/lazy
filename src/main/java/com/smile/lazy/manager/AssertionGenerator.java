...
public class AssertionGenerator {
    ...
    public void executeApiCall(ApiCall apiCall, LazyApiCallResponse lazyApiCallResponse, AssertionResultList assertionResultList) throws LazyException {
    ...
    private AssertionResult responseTimeAssertion(ApiCall apiCall, LazyApiCallResponse lazyApiCallResponse, AssertionRule assertionRule, AssertionOperationEnum operation) {
        AssertionResult assertionResult;
        AssertionValue assertionValue = assertionRule.getAssertionValue();
        String expectedValue = assertionValue.getExpectedValue1();
        long responseTime = lazyApiCallResponse.getResponseTime();
        assertionResult = new AssertionResult(1, apiCall.getApiCallId(), assertionRule.getAssertionRuleId(), Long.toString(responseTime));
        if (operation == AssertionOperationEnum.LESS_THAN) {
            ...
            assertionResult.setPass(responseTime > Long.parseLong(expectedValue) && responseTime < Long.parseLong(expectedValue2));
        } else {
    ...
    private AssertionResult responseCodeAssertion(ApiCall apiCall, LazyApiCallResponse lazyApiCallResponse, AssertionRule assertionRule, AssertionOperationEnum operation) {
        AssertionResult assertionResult;
        AssertionValue assertionValue = assertionRule.getAssertionValue();
        String expectedValue = assertionValue.getExpectedValue1();
        int responseCode = lazyApiCallResponse.getCloseableHttpResponse().getStatusLine().getStatusCode();
        assertionResult = new AssertionResult(1, apiCall.getApiCallId(), assertionRule.getAssertionRuleId(), Integer.toString(responseCode));
        if (operation == AssertionOperationEnum.EQUAL) {
            ...
            String expectedValue2 = assertionValue.getExpectedValue2();
            assertionResult.setPass(responseCode > Integer.parseInt(expectedValue) && responseCode < Integer.parseInt(expectedValue2));
        } else {
            LOGGER.warn("Assertion operation is not supported");
        }
        return assertionResult;
