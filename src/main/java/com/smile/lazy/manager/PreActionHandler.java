  ...
  public void executePreAction(LazySuite lazySuite, Action preAction) throws LazyException {
    ...
    if (preAction instanceof VariableDeclarationAction){
      ...
      if (ActionTypeEnum.SET_GLOBAL_VARIABLE == actionType) {
        lazySuite.getGlobalEnvironment().getEnvironmentVariableMap().put(variableDeclarationAction.getVariableKey(), environmentVariable);
        ...
        if (lazySuite.getEnvironments().containsKey(environmentDisplayId)){
          Environment environment = lazySuite.getEnvironments().get(environmentDisplayId);
